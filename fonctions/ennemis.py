...
def displayEnnemies(screen, WIDTH, SIZE):
    SPACE = 10
    X = (WIDTH - ((10 * SIZE) + (9 * SPACE))) // 2                     # // : division entière (sans reste)
    Y = 100
    ...
    displayLigne(ennemyStrong_img, X, Y, screen, SIZE, SPACE)
    displayLigne(ennemyMedium_img, X, (Y + (SIZE + SPACE)) , screen, SIZE, SPACE)
    displayLigne(ennemyMedium_img, X, (Y + (SIZE + SPACE) * 2), screen, SIZE, SPACE)
    displayLigne(ennemyWeak_img, X, (Y + (SIZE + SPACE) * 3), screen, SIZE, SPACE)
    displayLigne(ennemyWeak_img, X, (Y + (SIZE + SPACE) * 4), screen, SIZE, SPACE)
    displayLigne(ennemyWeak_img, X, (Y + (SIZE + SPACE) * 5), screen, SIZE, SPACE)
